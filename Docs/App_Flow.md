...
2.  **Role Assignment:** User maps specific roles (e.g., "Lead Writer", "Code Critic") to available models from the registry (React).
3.  **Config Persistence:** Frontend sends room config and model mapping to `POST /api/rooms` (React → Spring Boot).
4.  **Registry Binding:** Backend validates the mapping against the `Map<String, ChatClient>` registry (Spring Boot).
5.  **State Initialization:** A new `WorkflowState` object is created in PostgreSQL with status `INITIALIZED` (Spring Boot).
...
7.  **Client Sync:** Backend returns the `RoomDTO`; Frontend navigates to the chat view and opens a WebSocket connection to `/topic/room/{id}` (React).
...
2.  **Message Dispatch:** Frontend sends the command to the backend via `POST /api/chat/send` (React → Spring Boot).
3.  **Role Resolution:** Backend parses the message to identify the target model from the room's registry (Spring Boot).
...
5.  **Adapter Translation:** The canonical history is passed through the provider-specific Adapter:
    ...
    *   **OpenAI/Claude Adapters:** Maps to `user/assistant` schema (Spring Boot).
6.  **Inference Execution:**
    *   **Real Call (Gemini):** Dispatched via `VertexAiChatClient` (Spring Boot → Google API).
    *   **Mocked Call (OpenAI/Claude):** Dispatched to `FakeChatClient` for stubbed response simulation (Spring Boot).
...
9.  **Real-time Broadcast:** The new message and updated state summary are pushed to the room's WebSocket topic (Spring Boot → WebSocket).
...
2.  **State Trigger:** Any backend change (New Message, Status Change, Workflow Update) triggers a `RoomEvent` broadcast (Spring Boot).
3.  **Zustand Sync:** Upon receiving a WebSocket event, the local Zustand store updates the `messages` array and `currentWorkflow` object (React).
...
5.  **Consistency Check:** Periodically, the frontend polls the `GET /api/rooms/{id}/state` endpoint to resolve any dropped WebSocket frames (React → Spring Boot).
...
graph TD
    ...
    C --> D[Status: INTERVENTION_REQUIRED]
    D --> E[User Types Correction/Message]
    E -- POST /api/intervene --> F[Merge Message into WorkflowState]
...
6.  **Context Awareness:** The next model in the pipeline "sees" the user's intervention as the most recent context, allowing it to pivot based on the manual guidance (Real/Mocked Provider).