...
| Feature Name | Description | Inputs | Outputs | State Transitions | Failure Modes & Mitigations |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Schema Translation** (`ModelAdapter`) | Maps a flat history list (`CanonicalMessage`) and context summary (`WorkflowState`) into model-specific chat templates, resolving system prompt rules and context limits. | `List<CanonicalMessage>`, `WorkflowState`. | Model-specific prompt structure or formatted chat template string. | None (Stateless translation). | **Failure:** Mismatched chat template structure or validation failure. <br>**Mitigation:** Throws `TranslationException` and blocks invocation. |
