...
class FakeClaudeChatClientTest {

    private FakeClaudeChatClient client;

    @BeforeEach
    void setUp() {
        client = new FakeClaudeChatClient();
    }

    @Test
    void testCall_ReturnsMockResponseAndCalculatesTokens() {
        Prompt prompt = new Prompt("Review this java class.");
        ...
        assertTrue(text.contains("### Claude Code Critique"));
        assertTrue(text.contains("Memory leak risk"));
    ...
    void testStream_ReturnsReactiveFlux() {
        Prompt prompt = new Prompt("Critique code stream.");
        ...
        assertTrue(finalResult.contains("### Claude Code Critique"));
    }
}
